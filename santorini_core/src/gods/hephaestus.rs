...
build_power_move_generator!(
    heph_move_gen,
    build_winning_move: HephMove::new_winning_move,
    state: state,
    ...
    building_block: {
        for worker_build_pos in narrowed_builds {
            ...
            if (worker_build_mask & can_double_build_mask).is_not_empty() {
    ...
    extra_init: let can_double_build_mask = !state.board.at_least_level_2(),
);

pub const fn build_hephaestus() -> GodPower {
    god_power(
        GodName::Hephaestus,
        build_god_power_movers!(heph_move_gen),
