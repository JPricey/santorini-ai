...
// from(5)|to(5)|build(5)|is_double_build(1)
...
impl HephMove {
    ...
    pub fn new_heph_winning_move(move_from_position: Square, move_to_position: Square) -> Self {
        let data: MoveData = ((move_from_position as MoveData) << HEPH_MOVE_FROM_POSITION_OFFSET)
            | ((move_to_position as MoveData) << HEPH_MOVE_TO_POSITION_OFFSET)
...
fn heph_move_gen<const F: MoveGenFlags>(
    board: &BoardState,
    player: Player,
        ...
        for moving_worker_end_pos in worker_moves.into_iter() {
            ...
            if F & GENERATE_THREATS_ONLY != 0 {
                if is_already_check {
                    ...
                    worker_builds &= check_if_builds | check_if_double_builds;
                }
            }
            ...
            for worker_build_pos in worker_builds {
                ...
                if F & (INCLUDE_SCORE | GENERATE_THREATS_ONLY) != 0 {
                    ...
                    if F & GENERATE_THREATS_ONLY != 0 && score != CHECK_SENTINEL_SCORE {
                        // skip
                    } else {
                ...
                if (worker_build_mask & can_double_build_mask).is_empty() {
                    continue;
                }
                ...
                if F & (INCLUDE_SCORE | GENERATE_THREATS_ONLY) != 0 {
                    let score;
                    if is_already_check && (anti_check_builds & !worker_build_mask).is_not_empty()
                        || (worker_build_mask & check_if_double_builds).is_not_empty()
                    {
                        score = CHECK_SENTINEL_SCORE;
                    } else {
                        let is_improving = worker_end_height > worker_starting_height;
...
mod tests {
    ...
    fn test_heph_check_detection() {
        ...
        for state in game_state_fuzzer {
            ...
            let current_win = heph.get_winning_moves(&state.board, current_player);
            if current_win.len() != 0 {
                continue;
            }

            let actions = heph.get_moves_for_search(&state.board, current_player);
            for action in actions {
                let mut board = state.board.clone();
                heph.make_move(&mut board, action.action);

                let is_check_move = action.score == CHECK_SENTINEL_SCORE;
                let is_winning_next_turn = heph.get_winning_moves(&board, current_player).len() > 0;
    ...
    #[test]
    fn test_heph_improver_checks_only() {
        let heph = GodName::Hephaestus.to_power();
        let game_state_fuzzer = GameStateFuzzer::default();
        ...
        for state in game_state_fuzzer {
            ...
            let mut improver_moves = heph.get_improver_moves(&state.board, current_player);
            ...
            let mut all_moves = heph.get_moves_for_search(&state.board, current_player);
    ...
    fn test_check_detection_move_into() {
        let heph = GodName::Hephaestus.to_power();
        let state =
            FullGameState::try_from("11224 44444 00000 00000 00000/1/heph:A5,D5/heph:E1,E2")
                .unwrap();
        ...
        let actions = heph.get_moves_for_search(&state.board, Player::One);
        for action in actions {
            println!("{:?}", action);
        }
    }
