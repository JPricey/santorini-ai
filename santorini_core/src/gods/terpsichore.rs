...
impl TerpsichoreMove {
    ...
    pub fn new_winning_single_move(from1: Square, to1: Square, from2: Square, to2: Square) -> Self {
        let data: MoveData = ((from1 as MoveData) << MOVE_FROM_POSITION_OFFSET_1)
            | ((to1 as MoveData) << MOVE_TO_POSITION_OFFSET_1)
            | ((from2 as MoveData) << MOVE_FROM_POSITION_OFFSET_2)
            | ((to2 as MoveData) << MOVE_TO_POSITION_OFFSET_2)
...
fn handle_persephone_double_moves_for_move_masks<const F: MoveGenFlags>(
    ...
    for to1 in w1_moves {
        ...
        for to2 in final_moves_2 {
            ...
            let new_action =
                TerpsichoreMove::new_double_move(worker_start_1, to1, worker_start_2, to2);
...
fn handle_must_climb_double_moves<const F: MoveGenFlags>(
    ...
    let worker_start_1 = own_workers
        .next()
        .expect("Terpsichore must have 2 workers vs persephone");
    ...
    let improvers_1 = all_moves_1
        & match start_height_1 {
            ...
            _ => unreachable!(
                "Terpsichore should only have workers on level 0-2 vs persephone: {} {:?}",
                start_height_1,
                FullGameState::new(
                    prelude.board.clone(),
...
fn _push_harpy_double_move<const F: MoveGenFlags>(
    ...
    let new_action = TerpsichoreMove::new_double_move(w1_start, w1_dest, w2_start, w2_dest);
...
fn handle_harpies_double_moves<const F: MoveGenFlags>(
    ...
    let worker_start_1 = own_workers
        .next()
        .expect("Terpsichore must have 2 workers vs harpies");
    let worker_start_2 = own_workers
        .next()
        .expect("Terpsichore must have 2 workers vs harpies");
            ...
            if let Some(worker_start_2) = own_workers.next() {
                ...
                if (wins_2 & start_mask_1).is_not_empty() {
                    for to1 in moves_1 & !start_mask_2 {
                        let new_action = TerpsichoreMove::new_winning_double_move(
                            worker_start_1,
                            to1,
                            worker_start_2,
                            worker_start_1,
                ...
                if is_mate_only::<F>() {
                    ...
                    for to1 in moves_1 {
                        ...
                        for to2 in final_moves_2 {
                            ...
                            let new_action = TerpsichoreMove::new_double_move(
                                worker_start_1,
                                to1,
                                worker_start_2,
                                to2,
                ...
                if is_mate_only::<F>() {
                    ...
                    for to1 in final_moves_1 {
                        ...
                        let new_action = TerpsichoreMove::new_single_move(worker_start_1, to1);
    ...
    if let Some(worker_start_2) = worker_start_state.other_own_workers.into_iter().next() {
        ...
        for b1 in possible_builds_1 {
            ...
            for b2 in b2_builds {
                ...
                let new_action = TerpsichoreMove::new_double_build(b1, b2);
        ...
        for b1 in narrowed_builds {
            ...
            let new_action = TerpsichoreMove::new_single_build(b1);
            result.push(build_scored_move::<F, _>(new_action, is_check, false));
        }
    }
