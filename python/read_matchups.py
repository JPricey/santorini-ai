...
def main():
    ...
    # Always include the top 3 gods in "show losses" (what beats the strongest?)
    # and the bottom 3 in "show wins" (what do the weakest manage to beat?).
    SHOW_LOSSES_GODS = _dedup_preserve_order(SHOW_LOSSES_GODS + ordered_gods[:3])
    SHOW_WINS_GODS = _dedup_preserve_order(SHOW_WINS_GODS + ordered_gods[-3:])
...
def output_matchup_csv():
    ...
    god_winrates = [
        (get_god_wins[god] / get_god_present_in_matches[god], god)
        for god in get_god_present_in_matches.keys()
    ]
    ordered_god_names = list(
        god for _, god in sorted(god_winrates, key=lambda x: -x[0])
    )
    ...
    with open("tmp/matchup_chart.csv", "w", newline="") as f:
        writer = csv.writer(f)
        enumerated_god_names = [
            f"{i + 1}: {g}" for (i, g) in enumerate(ordered_god_names)
        ]
